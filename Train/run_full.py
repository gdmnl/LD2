...
if args.dev >= 0:
    ...
    for run in ['train', 'val', 'test']:
        for i, _ in enumerate(feat[run]):
            total_bytes += feat[run][i].element_size() * feat[run][i].nelement()
            feat[run][i] = feat[run][i].cuda(args.dev)
    labels = labels.cuda(args.dev)
torch.set_default_dtype(torch.float32)

optimizer = optim.Adam(model.parameters(), lr=args.lr*4, weight_decay=args.weight_decay/4)
...
def train(ld=feat['train'], lb=labels[idx['train']]):
    model.train()
    loss_list = []
    time_epoch = 0
    batch_xs, batch_y = ld, lb
...
for epoch in range(args.epochs):
    ...
    time_train += train_ep
    acc_val, _, _, _ = eval(ld=feat['val'], lb=labels[idx['val']])
...
start = time.time()
acc_test, time_inf, outl, labl = eval(ld=feat['test'], lb=labels[idx['test']])
...
print(f"Num params (M): {get_num_params(model):0.4f}, Mem params (MB): {get_mem_params(model):0.4f}, Mem Feat (MB): {total_bytes / 2**20:0.4f}")
